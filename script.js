...
let initialusers = [
  ...
  { id: 2, email: "logesh@user.com", password: "logesh@2001" },
];
...
const signIn = () => {
  ...
  if (Remail.value.length > 0 && Rpassword.value.length > 0) {
...
const signUp = () => {
  ...
  const emailR = document.getElementById("regemail");
  const passwordR = document.getElementById("regpassword");
  const confirmPasswordR = document.getElementById("confirmPassword");
  const errorR = document.getElementById("error");

  if (
    emailR.value.length > 0 &&
    passwordR.value.length > 0 &&
    nameR.value.length > 0 &&
    confirmPasswordR.value.length > 0
  ) {
    ...
    errorR.innerText = "Fields are empty";
  }
};
